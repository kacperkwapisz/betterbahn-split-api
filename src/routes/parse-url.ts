...
async function getResolvedUrlBrowserless(url: string) {
  ...
  const lastId =
    lastSegmentWithHalte.halte[lastSegmentWithHalte.halte.length - 1]?.id;
  if (!lastId) {
    throw new Error("Missing last halte id");
  }
  hashParams.set("zoid", lastId);
...
const handler = async (c: Context) => {
  ...
  if ("error" in journeyDetails) {
    return c.json({ error: journeyDetails.error });
  }
...
parseUrl.post("/", async (c) => {
  return await apiErrorHandler(() => handler(c));
});
